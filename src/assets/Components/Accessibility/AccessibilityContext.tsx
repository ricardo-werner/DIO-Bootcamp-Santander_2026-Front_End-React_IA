...
export function AccessibilityProvider({ children }: { children: ReactNode }) {
  ...
  const [fontSize, setFontSize] = useState<number>(() => Number(localStorage.getItem('acc_font_size')) || 100);
  const [lineHeight, setLineHeight] = useState<number>(() => Number(localStorage.getItem('acc_line_height')) || 1.6);
  const [letterSpacing, setLetterSpacing] = useState<number>(() => Number(localStorage.getItem('acc_letter_spacing')) || 0.02);
  const [isDyslexicFont, setIsDyslexicFont] = useState<boolean>(() => localStorage.getItem('acc_dyslexic_font') === 'true');
  const [darkMode, setDarkMode] = useState<boolean>(() => localStorage.getItem('acc_dark_mode') === 'true');
  const [colorBlindness, setColorBlindness] = useState<string>(() => localStorage.getItem('acc_color_blindness') || 'none');
  
  // NOVO: Armazena e inicializa a paleta de cores preferida
  const [colorPalette, setColorPalette] = useState<string>(() => localStorage.getItem('acc_color_palette') || 'calmo');
  ...
  }, [fontSize, lineHeight, letterSpacing, isDyslexicFont, darkMode, colorBlindness, colorPalette]);
